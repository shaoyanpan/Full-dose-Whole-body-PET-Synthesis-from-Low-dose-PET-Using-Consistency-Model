...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # cao = {"image":image['image'],'label':image['label']}
        ...
        if not self.train_flag:
            ...
            img = affined_data_dict['image']
            label = affined_data_dict['label']
            img_tensor = torch.unsqueeze(img, 1).to(torch.float)
            label_tensor = torch.unsqueeze(label, 1).to(torch.float)
